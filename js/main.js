...
function topNav() {
  ...
  if (nav) {
    ...
    let subMenu = cloneNav.querySelector('.submenuBox');
    subMenu.remove();
...
(function headLogo() {
  ...
  const logo = document.querySelector('.sideBox h1');
  const sideBox = document.querySelector('.sideBox');
  function fn() {
    windowWidthSmall < window.innerWidth ? sideBox.prepend(logo) : header.prepend(logo);
  }
...
function mainMenuSetup() {
  ...
  window.addEventListener('scroll', () => checkSideMenuHeight());
  ...
  const asideMenuPcLiHasChild = document.querySelectorAll('.sideBox .mainMenu li.hasChild > a');
  ...
  asideMenuPcLiHasChild.forEach((i) => {
    i.addEventListener('click', (e) => {
      i.parentNode.classList.contains('hasChild') ? e.preventDefault() : null;
      e.preventDefault();
      toggleAccordion(i, 'ul');
  ...
  function checkSideMenuHeight() {
    const sideMenu = document.querySelector('.mainMenu') || null;
    const objectRect = sideMenu?.getBoundingClientRect();
    objectRect.top > 0 ? (sideMenu.style.height = `${windowHeight - objectRect.top}px`) : null;
  }
